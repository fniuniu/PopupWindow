...
var GuaAlert2 = function(title, message, callback) {
    ...
    */
    ...
    <style class="modal-remove">
        ...
        .modal-control {
            font-size: 0;    /*var t父节点设置font-size：0，不然占位子，子元素才能设50%*/
        }
        .modal-button {
            width: 50%;
            height: 100%;
    ...
    $('.modal-button').on('click', function(event){
        ...
        var type = $(event.target).data('type')    //var type = $(this).data('type')
        if (type === 'cancel') {
            callback(false)
        } else {
            callback(true)
    ...
    */
    ...
    $('.modal-button').on('click', function(event){
        ...
        if (type === 'cancel') {
            ...
            var input = $('.modal-input').val()// 注意格式
            callback(true, input)
        }
        $('.modal-remove').remove()
    })
...
// 作业 4
//buttonTemplate函数生成一个按钮（包含名字和编号）
...
var GuaActions = function(title, actions, callback) {
    ...
    */
    ...
    var actionButtons = buttons.join('')   //让actionButtons是[按钮1按钮2...]
    var t = `
    <div class='modal-container modal-remove'>
        <div class='modal-mask'></div>
        <div class="modal-alert vertical-center">
    ...
    $('head').append(css)
    // event
    $('.modal-action-button').on('click', function(event){  //选中所有  button$('button').on
        console.log('click button')
